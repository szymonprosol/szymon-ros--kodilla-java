...
class BoardTestSuite {
    ...
    void testAddTaskListAverageWorkingOnTask() {
        ...
        List<TaskList> inProgressTasks = new ArrayList<>();
        inProgressTasks.add(new TaskList("In progress"));
        int quantityOfDays = project.getTaskLists().stream()
                .filter(inProgressTasks::contains)
                .flatMap(tl -> tl.getTasks().stream())
                .map(Task::getDays)
                .reduce(0, (sum, current) -> sum = sum + current);
        int quantityOfTasks = (int) project.getTaskLists().stream()
                .filter(inProgressTasks::contains)
                .flatMap(tl -> tl.getTasks().stream())
                .count();
        double agerage = (double)quantityOfDays / (double)quantityOfTasks;
        //Then
        Assertions.assertEquals(10, agerage);
    }
}